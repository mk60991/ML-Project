...
train=pd.DataFrame.from_csv("C:\\Users\\hp\\Downloads\\mAdhocProject\\kaggle-digit-recognizer\\data\\train-3.csv")
print (train.shape)
train=train.reset_index()
#print train.head(100)

test=pd.DataFrame.from_csv("C:\\Users\\hp\\Downloads\\mAdhocProject\\kaggle-digit-recognizer\\data\\test.csv")
print (test.shape)
test=test.reset_index()
#print test.head(100)

#Get unique pixel values in train and test
unique_train_pixels=pd.unique(train.loc[:, 'pixel0':'pixel783'].values.ravel())
...
model.fit(train_train.loc[:,'pixel0':'pixel783'], train_train.loc[:,'label'].values.ravel())
print ("model.score", model.score(train_test.loc[:,'pixel0':'pixel783'], train_test.loc[:,'label'].values.ravel()))
